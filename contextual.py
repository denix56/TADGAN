...
def contextual_prepare_weighted(expected, observed, data=None,
                                start=None, end=None):
    if data is not None:
        start = data['timestamp'].min()
        end = data['timestamp'].max()
    ...
    expected = _pad(expected)
    observed = _pad(observed)
    expected, observed, weights = _contextual_partition(expected, observed, start, end)
    return expected, observed, weights
...
def contextual_f1_score(expected, observed, data=None, start=None, end=None, weighted=True, beta=1):
    ...
    def _cm(x, y, z, w, f):
        return contextual_confusion_matrix(x, y, z, w, f, weighted)
    return _f1_score(expected, observed, data, start, end, _cm, beta)
